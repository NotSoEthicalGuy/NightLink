...
export default function Reservations() {
    ...
    // Filter to only show active reservations (PAID_CONFIRMED and PENDING_PAYMENT)
    const activeReservations = reservations.filter(r => 
        r.status === 'PAID_CONFIRMED' || r.status === 'PENDING_PAYMENT'
    );
    ...
    const getStatusStyles = (status: string) => {
        switch (status) {
            case 'PAID_CONFIRMED': return 'bg-green-500/10 text-green-500 border-green-500/20';
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-4xl font-display font-bold mb-1">Bookings</h1>
                    <p className="text-gray-500 font-medium">Full history and management of all spot reservations.</p>
                </div>
            ...
            <div className="bg-white/[0.02] border border-white/5 rounded-[40px] overflow-hidden">
                                ...
                                <tr key={res.id} className="hover:bg-white/[0.02] transition-colors group">
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center font-black text-xs text-purple-400 group-hover:bg-purple-500/20 transition-colors">
                ...
                {filteredReservations.length === 0 && (
                    <div className="text-center py-20">
                        <Ticket size={48} className="mx-auto text-gray-800 mb-4 opacity-20" />
                        <h4 className="text-lg font-bold text-gray-600">No bookings found</h4>
                        <p className="text-gray-700">Reservations will appear here once customers start booking.</p>
                    </div>
                )}
            </div>
        </div>
